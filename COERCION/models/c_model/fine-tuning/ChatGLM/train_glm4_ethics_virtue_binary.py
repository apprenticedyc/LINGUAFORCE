...
train_data_file = "/root/autodl-tmp/DATA/Ethics/virtue/virtue_train.csv"  # 替换为你的 Virtue CSV 路径
# 输出目录改名，以防覆盖之前的微调权重
output_dir = "/root/autodl-tmp/fine-tuning/Ethics/virtue/binary/glm4-lora-virtue-binary"
...
# ================= 3. 数据集准备 (适配 Ethics-Virtue) =================
def load_and_format_virtue(file_path):
    formatted_data = []
    print(f"[*] Reading Virtue data from {file_path}...")
    
    # 【防御式解析】极其严格的 CSV 读取限制，防止错行导致标签变文本
    df = pd.read_csv(
        file_path, 
        sep=',', 
        quotechar='"', 
        usecols=[0, 1],                # 强制只取前两列
        names=['label', 'scenario'],   # 重命名列，忽略原表头
        header=0,                      # 第一行视为表头剔除
    ...
    for _, row in df.iterrows():
        ...
        try:
            ...
            continue  # 遇到无法转为整型的脏数据直接跳过

        # 【核心技巧：Prompt 对齐】
        # 将情境伪装成对话的 Utterance 1
        prompt = (
...
train_dataset = load_and_format_virtue(train_data_file)
