import torch
import json
import os
from tqdm import tqdm
from transformers import AutoTokenizer, AutoModelForCausalLM
from peft import PeftModel

# ================= 1. 路径配置 =================
model_path = "/root/autodl-tmp/Llama-2-7b-chat-ms"  # 原始权重
lora_path = "/root/autodl-tmp/fine-tuning/multi/Llama7_LoRA/best_lora"  # 训练好的权重
test_data_path = "/root/autodl-tmp/DATA/test.jsonl"  # 测试集路径
output_json = "/root/autodl-tmp/fine-tuning/multi/Llama7_LoRA/results.jsonl"

# ================= 2. 加载模型与 Tokenizer =================
print("[*] Loading tokenizer...")
tokenizer = AutoTokenizer.from_pretrained(model_path, trust_remote_code=True)
# 【修改】显式设定 Llama-2 的 pad_token
tokenizer.pad_token = tokenizer.eos_token
# 【修改】推理阶段强烈建议使用左侧填充 (Left Padding)，避免破坏生成位置的绝对编码
tokenizer.padding_side = "left"

print("[*] Loading base model in bfloat16...")
# 直接以 bfloat16 加载基础模型，不使用任何量化
base_model = AutoModelForCausalLM.from_pretrained(
    model_path,
    torch_dtype=torch.bfloat16,
    device_map="auto",
    trust_remote_code=True
)

print("[*] Merging LoRA weights...")
# 加载 LoRA 适配器
model = PeftModel.from_pretrained(base_model, lora_path)
model.eval() # 切换到推理模式

# ================= 3. 推理函数 =================
def predict_severity(utterances):
    # 保持与训练时完全一致的清洗逻辑
    non_empty_utts = [utt for utt in utterances if utt.strip() != ""]
    dialogue_text = "\n".join([f"Utterance{j+1}: {utt}" for j, utt in enumerate(non_empty_utts)])

    # 保持与训练时完全一致的 Prompt
    prompt_multi = (
        "Below is a conversation. Please assess the severity of moral blackmail "
        "(0 indicates no blackmail, 5 indicates extreme blackmail).\n"
        "0: No blackmail, 1: Slight suspicion, 2: Slight tendency towards moral blackmail, "
        "3: Clear implication, 4: Highly self-centered, 5: Extreme blackmail.\n"
        f"Conversation:\n{dialogue_text}\n\n"
        "Your judgment (output only a number between 0 and 5):"
    )
    
    system_prompt = "You are a helpful assistant that analyzes conversations."

    # 【修改】弃用 apply_chat_template，直接使用 Llama-2 格式硬编码
    # 注意：推理时我们只给出用户的输入，以 [/INST] 结尾，引导模型开始回答
    text = f"<s>[INST] <<SYS>>\n{system_prompt}\n<</SYS>>\n\n{prompt_multi} [/INST]"
    
    inputs = tokenizer(text, return_tensors="pt").to(model.device)
    
    with torch.no_grad():
        outputs = model.generate(
            **inputs, 
            max_new_tokens=10, # 我们只需要输出一个数字，10个 token 足够了
            do_sample=False,   # 设为 False 相当于 Greedy Search，保证输出的确定性
            pad_token_id=tokenizer.eos_token_id
        )
    
    # 截断 Prompt 部分，只取生成的回复
    input_length = inputs.input_ids.shape[1]
    generated_ids = outputs[0][input_length:]
    response = tokenizer.decode(generated_ids, skip_special_tokens=True)
    
    return response.strip()

# ================= 4. 执行批量测试 =================
# 确保输出目录存在
os.makedirs(os.path.dirname(output_json), exist_ok=True)

results = []
print(f"[*] Starting inference on {test_data_path}...")

with open(test_data_path, "r", encoding="utf-8") as f:
    lines = f.readlines()

for line in tqdm(lines):
    data = json.loads(line)
    prediction = predict_severity(data["utterances"])
    
    # 保存结果以便后续计算准确率、F1 等指标
    results.append({
        "dialogue_id": data.get("dialogue_id", "N/A"),
        "ground_truth": str(data["dialog_multi_label"]),
        "prediction": prediction
    })

# ================= 5. 保存结果 =================
with open(output_json, "w", encoding="utf-8") as f:
    for res in results:
        f.write(json.dumps(res, ensure_ascii=False) + "\n")

print(f"[*] Inference finished. Results saved to {output_json}")