...
# 替换为你的 32B 模型实际路径
MODEL_PATH = "/root/autodl-tmp/GLM-4-32B-Chat" 
DATA_FILE = "/root/autodl-tmp/DATA/updated_test.jsonl"
BASE_OUTPUT_DIR = "/root/autodl-tmp/non-fine-tuning/ChatGLM/binary_ablation"
...
if __name__ == "__main__":
    ...
    print(f"Loading GLM-4 model with vLLM from: {MODEL_PATH}")
    # ================= 4. vLLM 引擎初始化 =================
    # llm = LLM(
    #     model=MODEL_PATH, 
    #     tensor_parallel_size=1,      
    #     pipeline_parallel_size=4,    # 适配 4 张显卡的流水线并行
    #     dtype="bfloat16",         
    #     trust_remote_code=True,      # GLM-4 必需
    #     gpu_memory_utilization=0.85, # 预留显存，防止长上下文 OOM
    #     max_model_len=2048,          # 根据你的数据调整，防止启动死锁
    #     enforce_eager=True           # 禁用 CUDA 图捕获，防止多卡通信卡死
    # )

    llm = LLM(
        model="/root/autodl-tmp/GLM-4-32B-Chat", 
        tensor_parallel_size=4,      # 4 张 5090 满血阵型
        dtype="bfloat16",            # 5090 原生支持且最高效的半精度格式
        trust_remote_code=True,      # 加载 GLM 的自定义分词器
        gpu_memory_utilization=0.90, # 显存极大，留 10% 防溢出即可
        max_model_len=8192,          # 剩余显存极大，可以放心开长上下文
        enforce_eager=True           # 多卡并行下，建议开启此项防止 CUDA Graph 死锁
    )
    ...
    for granularity in GRANULARITY_MODES:
        ...
        for combo in feat_combinations:
            ...
            for seed in SEEDS:
                # vLLM 需要在 SamplingParams 中透传 seed 才能保证生成一致性
