...
# 结果保存路径
BASE_OUTPUT_DIR = "/root/autodl-tmp/non-fine-tuning/Qwen25/binary"
...
# --- 特征定义库 (使用你提供的 Semantic Anchors 版本) ---
FEATURE_DEFINITIONS = {
...
def get_feature_display_str(val_dict):
    """
    仅处理结构化的 dict 数据 {'level': 'High', 'intensity_score': 0.8}
    """
...
def generate_prompt_content(item, active_indices, granularity):
    """
    完全基于 active_indices 和 granularity 动态生成 Prompt
    """
    
    # --- A. 生成定义部分 (Selection Logic) ---
    definitions = []
    ...
    if granularity in ["global_only", "both"]:
        lines = []
        overall = item.get("overall_analysis", {}) # 只读取 overall_analysis

        for idx in active_indices:
            name_key = FEATURE_NAMES[idx]
            
            # 如果该特征存在于 overall_analysis 中
    ...
    # 预处理 turn 数据，转为 dict: {turn_id: {feat_name: val_dict}}
    ...
    for i, text in enumerate(utterances):
        ...
        # 只有在 turn_only 或 both 模式下才嵌入特征
        if granularity in ["turn_only", "both"]:
            turn_signals = []
            
            # 仅检查 active_indices 里的特征
            for idx in active_indices:
                name_key = FEATURE_NAMES[idx]
...
if __name__ == "__main__":
    ...
    print("="*115)
    header = f"{'Mode':<12} | {'Features':<35} | {'Accuracy':<18} | {'Precision':<18} | {'Recall':<18} | {'F1 Score':<18}"
    print(header)
    print("-" * 115)
    ...
    for item in final_summary:
        ...
        print(f"{item['granularity']:<12} | {item['features']:<35} | {acc_str:<18} | {prec_str:<18} | {rec_str:<18} | {f1_str:<18}")
