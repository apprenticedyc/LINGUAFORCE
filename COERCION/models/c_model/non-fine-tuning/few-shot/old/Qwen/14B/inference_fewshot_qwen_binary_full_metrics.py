...
# ================= 3. 核心算法：分层强度平衡采样 =================
# (保持之前的逻辑不变，为了节省篇幅，这里简写，请确保复制了之前的完整函数)
...
# 结果存储结构: final_stats[shot_k] = {'acc': [], 'prec': [], 'rec': [], 'f1': []}
...
for k in SHOT_LIST:
    ...
    for seed in SEEDS:
        ...
        for item in test_data:
            ...
            pred = int(nums[0]) if nums else 0 # 如果解析失败，默认0，或者记录为-1并过滤
            
            y_true.append(item["dialog_binary_label"])
            y_pred.append(pred)
            
        # 4. 计算本轮指标 (使用 sklearn)
        # average='binary' 表示关注正类 (1) 的 P/R/F1
