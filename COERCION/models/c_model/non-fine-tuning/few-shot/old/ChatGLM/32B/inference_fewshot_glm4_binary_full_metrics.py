...
# ================= 2. 完整案例池 =================
# 注意：请确保你在实际运行前填入了你的 FULL_EXAMPLE_POOL 数据
FULL_EXAMPLE_POOL = [
...
print(f"Loading GLM-4-32B model with vLLM from {MODEL_NAME}...")

# 专为 4x4090 打造的满血极速配置
llm = LLM(
    model=MODEL_NAME, 
    tensor_parallel_size=4,      # 4 张 4090 齐上阵
    dtype="bfloat16",            # 满血半精度
    trust_remote_code=True,      # GLM 必需
    gpu_memory_utilization=0.90, # 留 10% 显存作为安全缓冲
    max_model_len=8192,          # 解锁长上下文
    enforce_eager=True           # 禁用 CUDA Graph 防死锁
)
tokenizer = llm.get_tokenizer()

# 在 vLLM 中，temperature=0.0 等价于你原来的 do_sample=False (贪婪解码)
...
for k in SHOT_LIST:
    ...
    for seed in SEEDS:
        ...
        # 2. 构造公共的 Base Messages (包含系统提示和所有 Few-shots)
        ...
        # 3. 组装当前 Seed 下所有的 Prompt 列表，交给 vLLM 批处理
        for item in test_data:
            ...
            # 使用 apply_chat_template 将消息字典转化为 GLM 标准字符串
            text = tokenizer.apply_chat_template(
                messages,
                tokenize=False,              # vLLM 需要接收字符串形式的 prompt 列表
...
print("\n\n" + "="*80)
print(f"{'FINAL EXPERIMENT REPORT (GLM-4-32B + vLLM)':^80}")
print("="*80)
