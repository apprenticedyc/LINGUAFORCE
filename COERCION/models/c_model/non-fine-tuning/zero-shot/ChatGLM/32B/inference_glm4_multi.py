...
# ================= 2. 初始化 vLLM =================
print(f"Loading model: {MODEL_PATH} with vLLM across 4 GPUs...")

# 专为 4 张显卡打造的满血张量并行配置
llm = LLM(
    model=MODEL_PATH, 
    tensor_parallel_size=4,      # 4 卡齐上阵，完美整除 65024 词表
    dtype="bfloat16",            # 满血半精度
    trust_remote_code=True,      # GLM 架构必需
    gpu_memory_utilization=0.90, # 留 10% 显存缓冲
...
for i, output in enumerate(outputs):
    ...
    if pred_multi != -1 and meta["true_multi"] != -1:
...
print("\n" + "="*50)
print("EXPERIMENT RESULTS (GLM-4-32B)")
print("="*50)
