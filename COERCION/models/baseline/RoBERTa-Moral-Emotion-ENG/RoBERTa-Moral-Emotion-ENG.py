...
MODEL_NAME = "D:/model/RoBERTa-Moral-Emotion-ENG"               # 请填写正确的模型 ID 或本地路径
SEEDS = [42, 520, 2026]
TEST_SIZE = 0.3
...
print(f"模型输出维度（类别数）: {NUM_LABELS}")

# ⚠️ 关键设定：根据模型是多标签还是单标签分类，选择激活函数
# 如果模型是多标签（每个样本可以有多个标签，如 Moral Foundations 的 10 个维度），设为 True
# 如果模型是单标签多分类（每个样本只有一个标签），设为 False
USE_SIGMOID = True   # 请根据实际情况修改
...
print("使用 RoBERTa-Moral-Emotion-ENG 提取特征...")
X = extract_features(texts)
print(f"特征提取完成，特征形状：{X.shape}")  # 应为 (样本数, NUM_LABELS)
