...
import numpy as np
import torch
from transformers import AutoTokenizer, AutoModelForSequenceClassification  # 使用 Auto 类加载
...
MODEL_NAME = "D:/model/deberta-v3-large-emotion"     # 实际 Hugging Face 模型 ID，如 "microsoft/deberta-v3-large-emotion"
SEEDS = [42, 520, 2026]
TEST_SIZE = 0.3
...
# 如果是多标签分类，通常使用 sigmoid；如果是单标签多分类，则使用 softmax
# 这里假设是多标签情感分类（如 GoEmotions），因此使用 sigmoid
USE_SIGMOID = True  # 如果模型是多标签，保持 True；若为单标签多分类，设为 False 并改用 softmax
...
def extract_features(texts, batch_size=16):
    ...
    for i in range(0, len(texts), batch_size):
        ...
        with torch.no_grad():
            ...
            if USE_SIGMOID:
                probs = torch.sigmoid(logits).cpu().numpy()  # 多标签概率
            else:
                probs = torch.softmax(logits, dim=-1).cpu().numpy()  # 单标签概率
        features.append(probs)
    return np.vstack(features)

print("使用情感模型提取特征...")
X = extract_features(texts)
print(f"特征提取完成，特征形状：{X.shape}")  # 应为 (样本数, NUM_EMOTIONS)
