...
class SemEvalDataset(Dataset):
    def __init__(self, dataframe, tokenizer, max_length=512):
        ...
        if len(self.samples) == 0:
            print(f"\n[WARNING] SemEval Dataset is empty after parsing!")
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            # 依然将多分类标签设为 -100，冻结 7 分类干扰
            'multi_label': torch.tensor(-100, dtype=torch.long),
            'binary_label': torch.tensor(sample["binary_label"], dtype=torch.long)
        }

def get_SemEval_loaders(args, tokenizer):
    """
    生成 SemEval-2019 的 DataLoader
    """
    print("[*] Loading SemEval-2019 datasets...")
    ...
    train_dataset = SemEvalDataset(df_train, tokenizer)
    valid_dataset = SemEvalDataset(df_valid, tokenizer)
    test_dataset = SemEvalDataset(df_test, tokenizer)
