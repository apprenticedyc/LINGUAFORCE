...
perlocutionary pressure appears along a broad spectrum of everyday
...
appear available. We call this capacity the \emph{\afd{}}.

Current NLP research fragments this spectrum across disconnected
...
benchmark with continuous, comparable measurements. As a result, no existing
resource answers three questions that matter for both science and safety:
...
them comparable; and (iii)~can a model trained on a unified spectrum
recognize manipulation forms it has never seen?



Only one point on this spectrum has been studied at scale---moral
coercion, in which a speaker exploits shared ethical norms and levies
preemptive moral labels such as ``selfish'' or ``apathetic''---where
decomposing the phenomenon into four psychological dimensions
(obligation, constraint, value judgement, toxicity) was shown to yield
more robust detection than direct classification~\cite{coercion}, and
it remains an open question whether such dimension-based decomposition
transfers to the wider spectrum of linguistic influence.

To address these gaps, we introduce \LNGF{}, a benchmark design for the
\afd{} in real-world multi-turn dialogues. Our contributions are threefold:
\begin{enumerate}
...
(Section~III), a universal inventory of seven psychological dimensions
(Section~III-B), and a linguistic-realization layer that links each dimension
to observable surface cues. The framework specializes to the obligation,
constraint, value-judgement, and toxicity channels previously studied for one
special case of the spectrum (moral coercion) as a consistency check.
...
consistency with existing labels on shared dialogues (Section~IV).
...
A discourse is \emph{agency-bearing} iff it (i)~targets a recognizable action
...
Figure~\ref{fig:framework} shows the architecture: Layer~1 is a functional
...
Moral coercion is a special case of this spectrum, located at high
$D_3$ (normative pressure) with moderate $D_1$ and low-to-moderate $D_7$
(explicitness).}
\label{fig:framework}
\end{figure*}
...
\label{sec:taxonomy}
Table~\ref{tab:taxonomy} presents the taxonomy. Four families organize
...
Prior-channel anchors show the correspondence to previously studied coercion
dimensions (OB/CS/VJ/TX).}
\label{tab:taxonomy}
\scriptsize
\begin{tabular}{@{}p{1.9cm}p{2.5cm}p{6.6cm}p{2.0cm}p{2.0cm}@{}}
\toprule
\textbf{Family} & \textbf{Type} & \textbf{Description and example} & \textbf{Prior} & \textbf{Role}\\
\midrule
...
\multirow{2}{*}{\parbox{1.7cm}{B. Exchange}}
 ...
 & B2 Threat/Warning & Harm-driven motivation. \emph{``If you do not sign, you will face consequences.''} & CS (explicit) & explicit constraint\\
\multirow{6}{*}{\parbox{1.7cm}{C. Social-normative}}
 & C1 Moral Appeal/Guilt & Appeals to conscience or moral duty. \emph{``A dutiful son would never let his mother suffer.''} & VJ+OB & normative core\\
 & C2 Obligation/Duty & Invokes role responsibilities. \emph{``You are the team leader; it is your duty.''} & OB & duty pressure\\
 & C3 Value Judgement/Shaming & Moral labeling of the target. \emph{``You are so selfish; you never think of others.''} & VJ & moral attack\\
...
\multirow{4}{*}{\parbox{1.7cm}{D. Covert}}
 ...
 & D3 Logical Trap/False Dilemma & Shrinks the option space. \emph{``Either you follow me or we are done.''} & CS & option elimination\\
 & D4 Verbal Abuse/Toxicity & Explicit hostility and disparagement. \emph{``You are useless; you cannot even do this.''} & TX & hostile pressure\\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{table}[t]
\centering
\caption{Seven psychological dimensions and their relation to previously
studied coercion channels (OB/CS/VJ/TX).}
\label{tab:dims}
\footnotesize
\begin{tabular}{@{}p{1.15cm}p{4.4cm}p{2.5cm}@{}}
\toprule
\textbf{Dim.} & \textbf{Definition} & \textbf{Prior}\\
\midrule
...
\LNGF{} re-annotates the real-world corpus of everyday multi-turn dialogues
...
Annotation follows a three-level decision tree: (1)~Does the discourse target
...
dimension, its Spearman correlation with the binary coercion label and the
...
We report: (i)~per-dimension means and consistency with the gold labels
(Table~\ref{tab:full}); (ii)~the dimension correlation matrix, testing
whether $D_3$--$D_4$ co-occur while $D_5$ stays orthogonal
(Figures~\ref{fig:dim-corr-full} and~\ref{fig:dim-corr-full}); (iii)~the
separation of coercive and non-coercive dialogues in the seven-dimensional
space (Figures~\ref{fig:box-full} and~\ref{fig:box-full}); and (iv)~the
monotonicity of predicted intensity in the gold 0--5 label
(Figures~\ref{fig:mono-full} and~\ref{fig:mono-full}). These analyses also serve
as sanity checks against spurious lexical correlations, a failure mode
identified in prior coercion datasets. Type-level analyses on the type-labeled split are reported in
...
All seven dimensions correlate with the binary coercion label and with the
...
moral coercion~\cite{coercion}. Each dimension separates
...
shows the lowest correlation with the coercion labels ($\rho{=}0.40$
...
\textbf{Data provenance and consent.} The dialogues inherit the source
corpus's de-identification process: the corpus was already anonymized by
prior work~\cite{coercion}, and this release adds no new personal
information. Collection of the underlying corpus complied with the platform
terms of service in force at the time; this paper performs re-annotation of
existing text and performs no new data collection, so no additional informed
consent is required beyond what the source project obtained.

\textbf{IRB and human subjects.} The study involves no human participants:
all annotations are produced by a frozen instruction-tuned LLM following a
public decision tree, and no behavioral or demographic data are collected.
A human spot-check of 120 dialogues was performed by the first author on
already-anonymized text and is not research on human subjects; an IRB review
is therefore not required under our institutional policy.

\textbf{Annotation welfare.} Although the bulk of annotation is
automated, the spot-check and any future human annotation expose annotators
to potentially distressing content (e.g., verbal abuse, guilt-tripping).
Annotators receive written guidelines, may skip any instance without
penalty, and take breaks; the released guidelines accompany the code
repository.

\textbf{Dual-use and abuse risk.} The taxonomy and dimensions could, in
principle, be used to craft more effective manipulative language. We release
the dataset and code for research purposes only, emphasize detection and
analysis rather than generation, and provide no generation recipes. The
dataset is not intended for automated moderation of real users without human
review, for surveillance, or for consequential decisions about individuals.

\textbf{Culture and language scope.} The current release is English and
dominated by everyday scenarios. The seven-dimension scheme is grounded in
pragmatics and persuasion research, but its cross-lingual and cross-cultural
validity is an open question; we caution against treating the dimensions as
culture-universal without further study.

\textbf{Data statement and maintenance.} A full datasheet~\cite{gebru2018datasheets}
accompanies the release (\texttt{datasheet.md}), documenting composition,
collection, preprocessing, intended uses, and known limitations. The dataset
is released under a research-only license; corrections are tracked as
...
existing coercion labels. We further specify three benchmark tasks, a
...
distinct from generic toxicity. Future work includes human inter-annotator agreement,
cross-lingual extension, and studying how agentive force interacts with
listener vulnerability and cultural norms.
