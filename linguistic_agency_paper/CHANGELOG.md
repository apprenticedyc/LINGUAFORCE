...
### 2026-08-27 — 论文表达增强
- 补强 abstract / introduction / discussion / conclusion 的主线叙事，明确 benchmark 同时服务检测与可解释分析。
- 收束局限表述，强调这是一个可被未来数据集修正的实用抽象，而非完整理论终局。
- 修正统计分析段和结论中的重复引用与排版小错（如 Spearman rho）。

### 2026-08-31 — 根据导师批注重构论文主线
- 将核心问题收束为「agentive force 的策略结构是什么，以及能否用共享维度测量」，并在引言、框架、数据集和实验各节补充 1--2 句章节主旨。
- 明确当前数据集覆盖的是负向、限制型 agentive force；透明请求/建议作为边界控制，表扬和鼓励等正向影响列为后续扩展，避免对“完整范围”作过强声称。
- 补充 speech-act theory 的理论衔接，重新解释四类策略和三层框架，删除“七维是心理学完备集合”的表述。
- 将实验小节改写为围绕可测量性、结构互补性、跨域有效性和实现稳健性的证据链，并保留模型多样性和更大规模人工验证作为后续工作。
- 用 Tectonic 重新编译：13 页，编译成功；仍有少量 IEEE 表格导致的轻微 overfull/underfull hbox 警告，不影响 PDF 生成。
- 第一批低成本修订：将“没有已有资源回答三个问题”改为较审慎的研究空白表述，删除贡献列表中的章节编号，并统一范围表述，避免把当前数据集说成覆盖完整 agentive-force 范围。
- 按导师意见移除师兄未发表论文作为本文已知参考的相关表述和引用；仅保留“现有去标识语料/来源标签”的客观数据来源说明。
- 将详细 Ethics and Privacy 移至正文末尾附录，正文保留数据范围与伦理摘要；附录位置经重新编译确认不影响实验和结论章节。
