# -*- coding: utf-8 -*-
"""Compare 7-dim conditioned vs direct on ALL 634 ids."""
import json, math
...
gold_bin = [direct[i]['gold_binary'] for i in ids]
gold_multi = [direct[i]['gold_multi'] for i in ids]
print("gold bin:", Counter(gold_bin))
...
metrics(d_pred_int, gold_multi, gold_bin, "DIRECT zero-shot")
metrics(c_pred_int, gold_multi, gold_bin, "7-DIM conditioned")

print("\nDirect pred bin dist:", Counter(int(direct[i]['is_moral_coercion']) for i in ids))
fpr = sum(1 for i in ids if direct[i]['gold_binary']==0 and int(direct[i]['is_moral_coercion'])==1)/max(sum(1 for i in ids if direct[i]['gold_binary']==0),1)
print(f"Direct FPR (pred=1 among gold=0) = {fpr:.3f}")
